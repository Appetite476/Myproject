...
int TcpSocket::sendMsg(char * sendData, int dataLen, int timeout)
{
	...
	if (ret == 0)
	{
		...
		if (netdata == NULL)
		{
			...
			m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, ret, "mlloc Err");
			return ret;
		}
	...
	if (ret < 0)
	{
		...
		{
			ret = TimeoutError;
			//printf("send message timeout err: %d\n ", ret);
			m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, ret, 
			"func writeTimeout() timeout");
			return ret;
		}
		return ret;
	}
...
int TcpSocket::recvMsg(char ** recvData, int & recvLen, int timeout)
{
	...
	if (recvData == NULL || timeout < 0)
	{
		ret = ParamError;
		m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, ret, 
		"func recvMsg() check recvData == NULL || timeout < 0");
		return ret;
	}
	...
	if (ret != 0)
	{
		...
		{
			ret = TimeoutError;
			m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, ret, "func readtimeout() timeout");
			return ret;
		}
		else
		{
			m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, ret, "func readtimeout() err");
			return ret;
		}
	}
...
int TcpSocket::blockIO(int fd)
{
	...
	{
		ret = flags;
		m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, ret, "func blockIO -> fcntl() err");
		return ret;
	}
...
int TcpSocket::noBlockIO(int fd)
{
	...
	{
		ret = flags;
		m_log.Log(__FILE__, __LINE__, ItcastLog::ERROR, ret, "func noblock ->fcntl() err");
		return ret;
	}
...
int TcpSocket::connectTimeout(sockaddr_in *addr, unsigned int wait_seconds)
{
	...
	if (ret < 0 && errno == EINPROGRESS)
	{
		...
		else if (ret == 1)
		{
			...
			/* ret返回为1（表示套接字可写），可能有两种情况，一种是连接建立成功，
			一种是套接字产生错误，*/
